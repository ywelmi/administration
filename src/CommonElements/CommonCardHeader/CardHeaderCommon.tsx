...
const CardHeaderCommon = (
  {
    ...
    headerClass,
    borderClass,
  }: CardHeaderCommonType,
) => {
  return (
    <CardHeader
      className={`${headClass} total-revenue ${
        !borderClass === true ? "card-no-border" : ""
      } `}
    >
      <H4>{title}</H4>
      {subTitle && <Link to={`/ecommerce/products`}>{subTitle}</Link>}
      {mainTitle &&
        (
