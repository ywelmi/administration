...
interface IListMartialArtMilitia {
  ...
  onRowSelect?: (
    row: TMartialArtMilitiaArmyGroupGet,
...
const userAction: ColumnDef<TMartialArtMilitiaArmyGroupGet> = {
  ...
  cell(props) {
    ...
    const { addMartialArtMilitias, deleteMartialArtMilitia } =
      useMartialArtMilitiaStore();
  ...
  return (
    <div className="table-responsive">
      <TanTable
        ref={tableRef}
        data={tableData}
        getRowId={getMartialArtMilitiaId}
...
const MartialArtMilitia = () => {
  const { MartialArtMilitias, addMartialArtMilitias } =
    useMartialArtMilitiaStore();
  ...
  const fetchDataGroup = useCallback(() => {
    const sportFilter = getMoreFilterByValue(
      "sport_id",
      "=",
      sportMartialArtMilitia?.id!
    );
    groupGetAll({ filter: `[${sportFilter}]` })
      .then((res) => {
        console.log("response here");
  ...
  useEffect(() => {
    (async () => {
      if (stateMartial == "manager") {
        ...
        const group = await groupGetAll({ filter: `[${allFilter}]` }).then(
          (res) => {
            const {
              data: { data },
            } = res;
      ...
      .then((res) => {
        ...
        if (status === 200) {
          if (data && sportMartialArtMilitia!.id) {
            ...
            lotsdrawScheduleGet(sportMartialArtMilitia!.id, content_id).then(
              (res) => {
                const { data, status } = res;
                if (status === 200)
  ...
  const fetch_data_content = () => {
    (async () => {
      const contents = await martialArtMilitiaArmyGroupGetContent(
        sportMartialArtMilitia!.id!
      ).then((res) => {
        return res.data;
      });
                ...
                <>
                  <div className="d-flex justify-content-center">
                    <H3 className="text-center">
                      Danh sách thi đấu võ Dân quân tự vệ
                    </H3>
                  </div>
                  <div className="d-flex justify-content-center">
                    <div
                ...
                {stateMartial == "manager" ? (
                  ...
                  <div className="d-flex justify-content-center mt-2">
                    <H3 className="text-danger text-center">
                      Chưa cập nhật bốc thăm nội dung
                    </H3>
                    <div
                      className="btn btn-warning text-dark"
                      onClick={() => {
