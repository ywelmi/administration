...
const GroupForm = ({ team: initTeam, onSubmit, onCancel, sportId }: IGroupForm) => {
    ...
    const fetch_data_content = () => {
        (async () => {
            const contents = await martialArtMilitiaArmyGroupGetContent(sportId).then((res) => {
                return res.data;
            });
            ...
            <Row className="g-3">
                ...
                <Col xs="12" className="gap-2" style={{ display: "flex" }}>
                    <Btn
                        ...
                        type="button"
                        onClick={() => {
                            formik.setFieldValue("name", orgName + " - " + formik.values.type);
                            formik.setFieldValue("sport_id", sportId);
                            ...
                            if (formik.values.type && formik.values.content_id) {
                                ...
                                toast.error("Vui lòng chọn đầy đủ các nội dung của nhóm thi đấu!");
                            }
                        }}
                    >
                        Xác nhận
